    ...
    <div className="rounded-xl border border-zinc-800/80 bg-zinc-950 p-5 transition-all">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-zinc-800/60 mb-5">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-medium bg-cyan-950/60 border border-cyan-800/60 text-cyan-300 px-2 py-0.5 rounded">
              Spécial COD
            </span>
          </div>
        ...
        <div className="flex items-center gap-2 self-start sm:self-auto">
          <div className="text-right">
            <span className="text-[10px] text-zinc-500 block uppercase tracking-wider">Taux de Réalisation Cash</span>
            <span className="text-sm font-bold font-mono text-emerald-400 tabular-nums">{overallCashConversion}%</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 mb-5">
        {stages.map((stage, idx) => (
          <div 
            key={stage.id} 
            className="bg-zinc-900/60 border border-zinc-800/70 rounded-lg p-2.5 flex flex-col justify-between relative overflow-hidden"
          >
            <div className="flex items-center justify-between mb-1">
              <span className="text-[10px] font-semibold text-zinc-400 truncate pr-1">
                {idx + 1}. {stage.name}
              </span>
              <span 
            ...
            {idx < stages.length - 1 && (
              <div 
                className="h-0.5 w-full mt-2 rounded-full bg-zinc-800" 
                style={{
                  ...
                  return (
                    <div className="rounded-lg border border-zinc-800 bg-zinc-950/95 p-3 shadow-2xl text-xs font-mono">
                      ...
                      <div className="text-zinc-400 py-0.5">
                        Volume : <span className="text-white font-bold">{data.count.toLocaleString()}</span>
                      </div>
                      <div className="text-zinc-400 py-0.5">
                        Rétention Étape Précédente : <span className="text-emerald-400 font-bold">{data.stepRate}%</span>
                      </div>
                    </div>
                  );
                }
