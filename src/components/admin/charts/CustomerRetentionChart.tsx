      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-100 dark:border-zinc-800/60 mb-5">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-medium bg-purple-50 border border-purple-200 text-purple-700 dark:bg-purple-950/60 dark:border-purple-800/60 dark:text-purple-300 px-2 py-0.5 rounded">
              Cohortes CRM
            </span>
          </div>
        ...
        <div className="flex items-center gap-3 self-start sm:self-auto font-mono">
          <div className="text-right">
            <span className="text-[10px] text-slate-400 dark:text-zinc-500 block uppercase">Taux Réachat 90J</span>
            <span className="text-sm font-bold text-purple-600 dark:text-purple-300 tabular-nums">+{latest.repeatPurchaseRate}%</span>
          </div>
          <div className="text-right border-l border-slate-200 dark:border-zinc-800 pl-3">
            <span className="text-[10px] text-slate-400 dark:text-zinc-500 block uppercase">LTV Moyen 90J</span>
            <span className="text-sm font-bold text-emerald-600 dark:text-emerald-400 tabular-nums">{latest.cumulativeLtv} {currency}</span>
          </div>
        </div>
      </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 10, right: 20, left: -20, bottom: 0 }}>
                ...
                if (active && payload && payload.length) {
                  return (
                    <div className="rounded-lg border border-slate-200 dark:border-zinc-800 bg-white/95 dark:bg-zinc-950/95 p-3 shadow-xl text-xs font-mono">
                      <p className="font-semibold text-slate-900 dark:text-zinc-200 mb-2 border-b border-slate-100 dark:border-zinc-800 pb-1">{label}</p>
                      <div className="text-purple-600 dark:text-purple-300 py-0.5 flex justify-between gap-4">
                        <span>Taux de Réachat :</span>
                        <span className="font-bold">{payload[0]?.value}%</span>
                      </div>
                      <div className="text-emerald-600 dark:text-emerald-400 py-0.5 flex justify-between gap-4">
            ...
            <Legend 
              ...
              formatter={(value) => <span className="text-slate-600 dark:text-zinc-400 text-xs">{value}</span>}
            />
            <Line
              yAxisId="rate"
              type="monotone"
