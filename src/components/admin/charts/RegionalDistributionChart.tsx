      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-zinc-800/60 mb-5">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-medium bg-blue-950/60 border border-blue-800/60 text-blue-300 px-2 py-0.5 rounded">
              12 Régions
            </span>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
                  ...
                  if (active && payload && payload.length) {
                    ...
                    return (
                      <div className="rounded-lg border border-zinc-800 bg-zinc-950/95 p-3 shadow-2xl text-xs font-mono">
                        ...
                        <p className="text-zinc-400">Part Commandes : <span className="text-white font-bold">{entry.value}%</span></p>
                        <p className="text-zinc-400">Chiffre d&apos;Affaires : <span className="text-emerald-400 font-bold">{entry.revenue.toLocaleString()} {currency}</span></p>
                        <p className="text-zinc-400">Taux Livraison : <span className="text-blue-400 font-bold">{entry.deliveryRate}%</span></p>
                      </div>
                    );
                  }
                  return null;
        ...
        <div className="space-y-2">
          {data.map((region) => (
            <div 
              key={region.name} 
              className="flex items-center justify-between p-2 rounded-lg bg-zinc-900/40 border border-zinc-800/60 text-xs"
            >
              <div className="flex items-center gap-2">
                <span className="h-2 w-2 rounded-full shrink-0" style={{ backgroundColor: region.color }} />
                <span className="font-medium text-zinc-200 truncate max-w-[140px]">{region.name}</span>
              </div>
              <div className="flex items-center gap-3 text-right font-mono">
                <span className="text-zinc-400 text-[11px]">{region.value}%</span>
                <span className="text-emerald-400 font-semibold text-[11px] tabular-nums">
