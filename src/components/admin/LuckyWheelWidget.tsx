        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-150">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-md w-full p-6 text-center shadow-2xl relative space-y-5">
            ...
            <div className="relative w-64 h-64 mx-auto flex items-center justify-center">
              {/* Pointer Arrow */}
              <div className="absolute -top-2 z-20 w-0 h-0 border-l-[10px] border-l-transparent border-r-[10px] border-r-transparent border-t-[18px] border-t-emerald-400 filter drop-shadow" />

              {/* Wheel Disk */}
              <div
                className="w-56 h-56 rounded-full border-4 border-emerald-500/40 shadow-2xl relative overflow-hidden transition-transform duration-[3500ms] ease-out"
                style={{
                  transform: `rotate(${rotation}deg)`,
                  background: 'conic-gradient(#f43f5e 0deg 60deg, #059669 60deg 120deg, #2563eb 120deg 180deg, #0ea5e9 180deg 240deg, #8b5cf6 240deg 300deg, #10b981 300deg 360deg)',
                }}
              >
                {/* Center Hub */}
                <div className="absolute inset-0 m-auto w-12 h-12 rounded-full bg-slate-950 border-2 border-emerald-400 flex items-center justify-center text-xs font-black text-emerald-400 shadow-inner z-10">
                  COD
                </div>
              </div>
            </div>
            ...
            <div className="pt-2">
              <button
                ...
                disabled={spinning}
                onClick={handleSpin}
                className="w-full py-3 px-4 bg-emerald-500 hover:bg-emerald-400 disabled:opacity-50 text-zinc-950 font-black text-sm rounded-xl shadow-lg shadow-emerald-500/20 transition cursor-pointer"
              >
