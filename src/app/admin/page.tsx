    ...
    <div className="p-6 sm:p-10 space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black text-white tracking-tight">
            Tableau de Bord — {storeSlug.toUpperCase()}
          </h1>
          <p className="text-slate-400 text-xs sm:text-sm mt-1">
            Gérez vos commandes Cash on Delivery et vos expéditions au Maroc.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <Palette className="w-4 h-4" /> Modifier le Page Builder
          </Link>
          <Link
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-5 rounded-2xl bg-slate-900 border border-slate-800 space-y-2">
          ...
          <div className="text-[11px] text-slate-400">
            Taux de retour contenu à {analytics.returnRate}%
          </div>
        </div>
        ...
        <div className="p-5 rounded-2xl bg-amber-500/10 border border-amber-500/30 space-y-2">
          ...
          <div className="text-2xl sm:text-3xl font-black text-white">
            {pendingOrders.length} <span className="text-xs font-normal text-amber-300">commandes</span>
          </div>
          <Link
          ...
          >
            Appeler les clients maintenant &rarr;
          </Link>
        </div>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-base font-extrabold text-white">Dernières Commandes Clients</h2>
            <p className="text-xs text-slate-400">Mises à jour en direct depuis votre boutique</p>
          </div>
          <Link
            href={`/admin/orders?store=${storeSlug}`}
            className="text-xs font-bold text-amber-400 hover:underline"
          >
            Voir toutes les commandes ({orders.length}) &rarr;
          </Link>
        </div>
          ...
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="border-b border-slate-800 text-slate-400 font-semibold">
                ...
                <th className="py-3 px-3">Total (MAD)</th>
