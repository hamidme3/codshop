    ...
    <div className="p-4 sm:p-6 md:p-10 space-y-6 max-w-7xl mx-auto font-sans">
                    ...
                    <div className="space-y-3 pt-3 border-t border-zinc-800/80">
                      ...
                      <div
                        ...
                        className={`relative flex flex-col items-center justify-center p-4 rounded-xl border-2 border-dashed transition-all cursor-pointer ${
                          isDraggingAdd
                            ...
                            : 'border-slate-300 hover:border-emerald-500 dark:border-zinc-800 dark:hover:border-zinc-700 bg-slate-50 hover:bg-slate-100/80 dark:bg-zinc-950/40 dark:hover:bg-zinc-900/40'
                        }`}
                      >
                        <input
                          ref={addFileInputRef}
                        ...
                        />
                        {isUploadingAddImage ? (
                          <div className="flex items-center gap-2 text-emerald-600 dark:text-emerald-400 py-2 text-xs font-medium">
                            <Loader2 className="w-4 h-4 animate-spin" />
                            <span>Téléversement et optimisation des photos en cours...</span>
                          </div>
                        ) : (
                          <div className="text-center space-y-1">
                            <div className="inline-flex p-2 rounded-lg bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 text-emerald-600 dark:text-emerald-400 mb-0.5 shadow-xs">
                              <UploadCloud className="w-5 h-5" />
                            </div>
                            <div className="text-xs text-slate-700 dark:text-zinc-300 font-medium">
                              <span>Glissez vos photos ici ou </span>
                              <span className="text-emerald-600 dark:text-emerald-400 underline underline-offset-2 font-semibold">parcourir les fichiers</span>
                            </div>
                      ...
                      <div className="space-y-1.5 pt-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] text-slate-500 dark:text-zinc-500 uppercase font-mono tracking-wider">ou ajouter par URL web</span>
                          <div className="h-px flex-1 bg-slate-200 dark:bg-zinc-800/80" />
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            className="flex-1 bg-white dark:bg-[#0d0d10] border border-slate-300 dark:border-zinc-800 rounded-lg px-3 py-2 text-slate-900 dark:text-zinc-200 placeholder-slate-400 dark:placeholder-zinc-600 text-xs font-mono focus:outline-none focus:border-emerald-500/60"
                          />
                          <button
                            type="button"
                            onClick={() => handleAddImageToAdd()}
                            className="px-3.5 py-2 rounded-lg bg-slate-900 hover:bg-slate-800 text-white dark:bg-zinc-800 dark:hover:bg-zinc-700 dark:text-zinc-200 text-xs font-semibold border border-slate-800 dark:border-zinc-700 transition-colors cursor-pointer shadow-xs"
                          >
                            + Ajouter
                          </button>
                        </div>
              ...
              <div className="p-4 border-t border-zinc-800/80 bg-[#0f0f12] flex items-center justify-between">
                ...
                <div className="flex items-center gap-2.5">
                  <button
                    type="button"
                    onClick={() => setShowAddModal(false)}
                    className="px-4 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200 dark:bg-zinc-900 dark:hover:bg-zinc-800 dark:text-zinc-300 dark:border-zinc-800 text-xs font-medium transition-colors cursor-pointer"
                  >
                    Annuler
                  </button>
                  <button
                    type="submit"
                    className="px-5 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white dark:bg-emerald-500 dark:hover:bg-emerald-400 dark:text-zinc-950 font-bold text-xs transition-colors shadow-sm cursor-pointer flex items-center gap-1.5"
                  >
      ...
      {showEditModal && editingProduct && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-[#121215] border border-slate-200 dark:border-zinc-800 rounded-xl p-6 max-w-xl w-full max-h-[90vh] overflow-y-auto space-y-4 shadow-2xl admin-scrollbar">
            <div className="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-zinc-800/80">
              <div className="flex items-center gap-2.5">
                <div className="w-8 h-8 rounded-lg bg-slate-100 dark:bg-zinc-800/80 border border-slate-200 dark:border-zinc-700/80 flex items-center justify-center">
                  <Edit3 className="w-4 h-4 text-slate-700 dark:text-zinc-300" />
                </div>
                <div>
                  <h3 className="text-base font-semibold text-slate-900 dark:text-white">Modifier le Produit</h3>
                  <div className="text-[11px] font-mono text-slate-500 dark:text-zinc-400">{editingProduct.sku}</div>
                </div>
              </div>
              <button 
                type="button"
                onClick={() => setShowEditModal(false)} 
                className="text-slate-400 hover:text-slate-700 dark:hover:text-white p-1 rounded-lg hover:bg-slate-100 dark:hover:bg-zinc-800 transition-colors cursor-pointer"
              >
                ✕
              </button>
            </div>
            ...
            <form onSubmit={handleSaveEditProduct} className="space-y-3.5 text-xs">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <button
                      type="button"
                      onClick={() => setShowAddCategoryModal(true)}
                      className="text-[11px] text-emerald-600 dark:text-zinc-400 hover:text-emerald-700 dark:hover:text-zinc-200 font-medium inline-flex items-center gap-0.5"
                    >
                ...
                <div>
                  <label className="block text-slate-700 dark:text-zinc-300 font-medium mb-1">Stock Global :</label>
                  <input
                    type="number"
                    value={editStock}
                    onChange={(e) => setEditStock(Number(e.target.value))}
                    className="w-full bg-slate-50 dark:bg-[#0d0d10] border border-slate-300 dark:border-zinc-800 rounded-lg p-2.5 text-slate-900 dark:text-zinc-100 font-mono tabular-nums focus:outline-none focus:border-emerald-500 font-semibold"
                  />
                </div>
              </div>

              {/* Pricing & Dynamic Margin Calculation */}
              <div className="p-3.5 rounded-lg bg-slate-50 dark:bg-[#0d0d10] border border-slate-200 dark:border-zinc-800/80 space-y-2.5">
                <div className="grid grid-cols-3 gap-2.5">
                  <div>
                    <label className="block text-slate-600 dark:text-zinc-400 font-medium mb-1">Prix Vente (DH) :</label>
                    <input
                      type="number"
                      value={editPrice}
                      onChange={(e) => setEditPrice(Number(e.target.value))}
                      className="w-full bg-white dark:bg-zinc-900 border border-slate-300 dark:border-zinc-800 rounded-lg p-2 text-slate-900 dark:text-zinc-100 font-mono tabular-nums font-semibold text-xs focus:outline-none focus:border-emerald-500"
                    />
                  </div>

                  <div>
                    <label className="block text-slate-600 dark:text-zinc-400 font-medium mb-1">Prix Barré (DH) :</label>
                    <input
                      type="number"
                      value={editComparePrice}
                      onChange={(e) => setEditComparePrice(Number(e.target.value))}
                      className="w-full bg-white dark:bg-zinc-900 border border-slate-300 dark:border-zinc-800 rounded-lg p-2 text-slate-500 dark:text-zinc-400 font-mono tabular-nums text-xs focus:outline-none focus:border-emerald-500"
                    />
                  </div>

                  <div>
                    <label className="block text-slate-600 dark:text-zinc-400 font-medium mb-1">Coût Achat (DH) :</label>
                    <input
                      type="number"
                      value={editCostPrice}
                      onChange={(e) => setEditCostPrice(Number(e.target.value))}
                      className="w-full bg-white dark:bg-zinc-900 border border-slate-300 dark:border-zinc-800 rounded-lg p-2 text-emerald-600 dark:text-emerald-400 font-mono tabular-nums font-semibold text-xs focus:outline-none focus:border-emerald-500"
                    />
                  </div>
                </div>

                <div className="flex items-center justify-between px-3 py-1.5 rounded-md bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-800/40 text-emerald-800 dark:text-emerald-400 text-xs">
              ...
              <div className="space-y-3 p-3.5 rounded-lg bg-slate-50 dark:bg-[#0d0d10] border border-slate-200 dark:border-zinc-800/80">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <div className="flex items-center justify-between sm:justify-start gap-2">
                    <div className="font-medium text-slate-900 dark:text-zinc-200 text-xs flex items-center gap-1.5">
                      <Layers className="w-3.5 h-3.5 text-slate-500 dark:text-zinc-400 shrink-0" />
                      <span className="font-bold">Variantes & Stocks Détaillés</span>
                      <span className="px-2 py-0.5 rounded bg-slate-200 dark:bg-zinc-800 text-slate-700 dark:text-zinc-300 font-mono text-[10px] whitespace-nowrap font-medium">
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    <button
                      ...
                      className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-[11px] font-medium border transition-colors whitespace-nowrap cursor-pointer ${
                        showMatrixTools
                          ? 'bg-slate-200 text-slate-900 border-slate-300 dark:bg-zinc-700 dark:text-white dark:border-zinc-600'
                          : 'bg-white hover:bg-slate-100 text-slate-700 border-slate-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 dark:text-zinc-300 dark:border-zinc-700'
                      }`}
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-white hover:bg-slate-100 text-slate-800 dark:bg-zinc-800 dark:hover:bg-zinc-700 dark:text-zinc-200 font-medium text-[11px] border border-slate-200 dark:border-zinc-700 transition-colors whitespace-nowrap cursor-pointer"
                    >
                ...
                {showMatrixTools && (
                  <div className="p-3 rounded-lg bg-white dark:bg-zinc-900/90 border border-slate-200 dark:border-zinc-700/80 space-y-2.5 animate-in fade-in duration-150">
                    <div className="text-[11px] font-semibold text-slate-800 dark:text-zinc-300 flex items-center justify-between">
                      <span>⚡ Générateur Cartésien & Remplissage en Lot</span>
                      <span className="text-[10px] text-slate-500 dark:text-zinc-500 font-normal">Calcul instantané & SKUs propres</span>
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                      <div>
                        <label className="block text-[10px] text-slate-600 dark:text-zinc-400 font-medium mb-1">
                        ...
                        <input
                          ...
                          placeholder="Noir Ébène, Marron Vintage, Camel"
                          className="w-full bg-slate-50 dark:bg-[#0d0d10] border border-slate-300 dark:border-zinc-800 rounded px-2.5 py-1 text-slate-900 dark:text-zinc-100 placeholder-slate-400 dark:placeholder-zinc-600 text-xs"
                        />
                      </div>
                      <div>
                        <label className="block text-[10px] text-slate-600 dark:text-zinc-400 font-medium mb-1">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2 pt-1 border-t border-slate-200 dark:border-zinc-800/80">
                      <div className="flex items-center gap-1.5">
                        <label className="text-[10px] text-slate-600 dark:text-zinc-400 font-medium">Stock par variante :</label>
                        <input
                          type="number"
                          value={batchStockValue}
                          onChange={(e) => setBatchStockValue(Number(e.target.value))}
                          className="w-16 bg-white dark:bg-[#0d0d10] border border-slate-300 dark:border-zinc-800 rounded px-2 py-0.5 text-slate-900 dark:text-zinc-100 text-center font-mono tabular-nums text-xs"
                        />
                        <button
                          type="button"
                          onClick={handleApplyBatchStock}
                          className="px-2 py-1 rounded bg-slate-100 hover:bg-slate-200 text-slate-700 dark:bg-zinc-800 dark:hover:bg-zinc-700 dark:text-zinc-200 text-[10px] font-medium border border-slate-200 dark:border-zinc-700"
                      ...
                      <button
                        ...
                        className="px-2.5 py-1 rounded bg-slate-900 hover:bg-slate-800 text-white dark:bg-zinc-100 dark:hover:bg-white dark:text-zinc-900 text-[11px] font-semibold transition-colors shadow-xs"
                      >
                ...
                {/* Variants List */}
                <div className="space-y-2 max-h-56 overflow-y-auto pr-1 admin-scrollbar">
                  {editVariants.map((v, idx) => (
                    <div
                      key={idx}
                      className="p-2 rounded-lg bg-white dark:bg-zinc-900/90 border border-slate-200 dark:border-zinc-800 space-y-1.5 sm:space-y-0 sm:flex sm:items-center sm:gap-2"
                    >
                      <div className="flex items-center gap-1.5 flex-1 min-w-0">
                        {v.sku && (
                          <span className="hidden sm:inline-block px-1.5 py-0.5 rounded bg-slate-100 dark:bg-zinc-800 text-slate-600 dark:text-zinc-400 font-mono text-[9px] truncate max-w-[80px]" title={v.sku}>
                            {v.sku}
                          </span>
                        )}
                        <input
                          ...
                          className="w-1/2 sm:w-auto sm:flex-1 bg-slate-50 dark:bg-[#0d0d10] border border-slate-300 dark:border-zinc-800 rounded-md px-2.5 py-1 text-slate-900 dark:text-zinc-100 placeholder-slate-400 dark:placeholder-zinc-600 text-xs min-w-0"
                        />
                        <input
                          type="text"
                          placeholder="Taille / Réf (ex: 42)"
                          value={v.size || ''}
                          onChange={(e) => handleUpdateEditVariant(idx, 'size', e.target.value)}
                          className="w-1/2 sm:w-auto sm:flex-1 bg-slate-50 dark:bg-[#0d0d10] border border-slate-300 dark:border-zinc-800 rounded-md px-2.5 py-1 text-slate-900 dark:text-zinc-100 placeholder-slate-400 dark:placeholder-zinc-600 text-xs min-w-0"
                        />
                      </div>
                      <div className="flex items-center justify-between sm:justify-end gap-2 shrink-0 pt-1 sm:pt-0 border-t sm:border-t-0 border-slate-200 dark:border-zinc-800/60">
                        <div className="flex items-center gap-1.5">
                          <span className="text-[10px] text-slate-500 dark:text-zinc-500 sm:hidden">Stock :</span>
                          <input
                            type="number"
                            placeholder="Stock"
                            value={v.stock}
                            onChange={(e) => handleUpdateEditVariant(idx, 'stock', Number(e.target.value))}
                            className="w-16 bg-white dark:bg-[#0d0d10] border border-slate-300 dark:border-zinc-800 rounded-md px-2 py-1 text-slate-900 dark:text-zinc-100 text-center font-mono tabular-nums font-semibold text-xs"
                          />
                          <span className="text-[10px] text-slate-500 dark:text-zinc-500 font-mono">un.</span>
                        </div>
                        <button
                          type="button"
                          onClick={() => handleRemoveEditVariant(idx)}
                          className="p-1 rounded text-slate-400 hover:text-rose-600 hover:bg-rose-50 dark:text-zinc-500 dark:hover:text-rose-400 dark:hover:bg-rose-950/40 transition-colors cursor-pointer"
                          title="Supprimer cette variante"
                        >
                          <Trash2 className="w-3.5 h-3.5" />
                        </button>
              ...
              <div className="space-y-2.5 p-3.5 rounded-lg bg-slate-50 dark:bg-[#0d0d10] border border-slate-200 dark:border-zinc-800/80">
                <div className="flex items-center justify-between">
                  <div className="font-medium text-slate-900 dark:text-zinc-200 text-xs flex items-center gap-1.5">
                ...
                <div className="flex flex-wrap items-center gap-1.5">
                  <span className="text-[10px] text-slate-500 dark:text-zinc-500 font-medium">Presets Marocains :</span>
                  {MOROCCAN_PRODUCT_IMAGE_PRESETS.map((preset) => (
                    <button
                      key={preset.id}
                      type="button"
                      onClick={() => handleApplyImagePreset(preset)}
                      className="px-2 py-0.5 rounded bg-white hover:bg-slate-100 text-slate-700 dark:bg-zinc-800 dark:hover:bg-zinc-700 dark:text-zinc-300 text-[10px] border border-slate-200 dark:border-zinc-700/80 transition-colors shadow-2xs"
                      title={preset.title}
                    >
                      + {preset.niche}
                    </button>
                      ...
                      return (
                        <div
                          ...
                          className={`relative rounded-lg overflow-hidden border ${
                            isPrimary ? 'border-emerald-500/80 ring-1 ring-emerald-500/50' : 'border-slate-200 dark:border-zinc-800'
                          } bg-slate-100 dark:bg-zinc-900 group`}
                        >
                          <img
                            src={imgUrl}
                            alt={`Photo ${idx + 1}`}
                ...
                <div className="space-y-1.5 pt-1">
                  ...
                  <div className="flex items-center gap-1.5">
                    <input
                      ...
                      className="flex-1 bg-white dark:bg-[#0d0d10] border border-slate-300 dark:border-zinc-800 rounded-md px-2.5 py-1.5 text-slate-900 dark:text-zinc-200 placeholder-slate-400 dark:placeholder-zinc-600 text-xs font-mono focus:outline-none focus:border-emerald-500"
                    />
                    <button
                      type="button"
                      onClick={handleAddImage}
                      className="px-3 py-1.5 rounded-md bg-slate-900 hover:bg-slate-800 text-white dark:bg-zinc-800 dark:hover:bg-zinc-700 dark:text-zinc-200 text-xs font-semibold border border-slate-800 dark:border-zinc-700 shadow-xs cursor-pointer transition-colors"
                    >
                      + Ajouter
                    </button>
                  </div>
              ...
              <div className="flex gap-2 pt-2 border-t border-slate-200 dark:border-zinc-800/80">
                <button
                  ...
                  className="flex-1 py-2 px-4 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white dark:bg-emerald-500 dark:hover:bg-emerald-400 dark:text-zinc-950 font-bold text-xs transition-colors shadow-sm disabled:opacity-50 flex items-center justify-center gap-2 cursor-pointer"
                >
                  {isSavingEdit ? (
                    <>
                ...
                <button
                  ...
                  disabled={isSavingEdit}
                  onClick={() => setShowEditModal(false)}
                  className="py-2 px-4 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200 dark:bg-zinc-900 dark:hover:bg-zinc-800 dark:text-zinc-300 dark:border-zinc-800 text-xs font-medium transition-colors disabled:opacity-50 cursor-pointer"
                >
                  Annuler
                </button>
              </div>
      ...
      {showAICoach && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#121215] border border-slate-200 dark:border-zinc-800 rounded-xl p-6 sm:p-7 max-w-2xl w-full max-h-[90vh] overflow-y-auto space-y-5 shadow-2xl admin-scrollbar">
            {/* Modal Header */}
            <div className="flex items-start justify-between pb-3.5 border-b border-slate-200 dark:border-zinc-800/80">
              <div>
                <div className="flex items-center gap-2">
                  <span className="px-2 py-0.5 rounded text-[10px] font-mono uppercase tracking-wider bg-slate-100 text-slate-700 border border-slate-200 dark:bg-zinc-800 dark:text-zinc-300 dark:border-zinc-700">
                    Assistant Marketing Maroc
                  </span>
                </div>
                <h3 className="text-base sm:text-lg font-semibold text-slate-900 dark:text-white mt-1 flex items-center gap-2">
                  <Sparkles className="w-4 h-4 text-emerald-600 dark:text-emerald-400" /> Coach IA Marocain (Darija / FR)
                </h3>
                <p className="text-slate-500 dark:text-zinc-400 text-xs mt-0.5">
            ...
            {/* Generator Controls */}
            <div className="space-y-3.5 bg-slate-50 dark:bg-[#0d0d10] p-4 rounded-xl border border-slate-200 dark:border-zinc-800/80 text-xs">
              <div>
                <label className="block text-slate-600 dark:text-zinc-400 font-medium mb-1.5">Secteur / Niche :</label>
                  ...
                  {MOROCCAN_NICHES.map((niche) => (
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-md text-xs font-medium transition-colors cursor-pointer ${
                        aiNiche === niche
                          ? 'bg-slate-900 text-white dark:bg-zinc-100 dark:text-zinc-900 font-semibold shadow-xs'
                          : 'bg-white text-slate-700 hover:text-slate-900 border border-slate-200 dark:bg-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-200 dark:border-zinc-800'
                      }`}
                    >
                      {niche}
                    </button>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                <div>
                  <label className="block text-slate-600 dark:text-zinc-400 font-medium mb-1">Nom ou mot-clé :</label>
                  <input
                    type="text"
                    value={aiKeyword}
                    onChange={(e) => setAiKeyword(e.target.value)}
                    placeholder="Ex: Sacoche Cuir Marron"
                    className="w-full bg-white dark:bg-[#121215] border border-slate-300 dark:border-zinc-800 rounded-lg px-3 py-1.5 text-slate-900 dark:text-zinc-100 placeholder-slate-400 dark:placeholder-zinc-500 focus:outline-none focus:border-emerald-500"
                  />
                </div>
                <div>
                  <label className="block text-slate-600 dark:text-zinc-400 font-medium mb-1">Prix de Vente Ciblé (DH) :</label>
                  <input
                    type="number"
                    value={aiPrice}
                    onChange={(e) => setAiPrice(Number(e.target.value))}
                    className="w-full bg-white dark:bg-[#121215] border border-slate-300 dark:border-zinc-800 rounded-lg px-3 py-1.5 text-slate-900 dark:text-zinc-100 font-mono tabular-nums font-semibold focus:outline-none focus:border-emerald-500"
                  />
                </div>
              </div>
              ...
              <button
                ...
                className="w-full py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-800 dark:bg-zinc-800 dark:hover:bg-zinc-700 dark:text-zinc-200 font-medium text-xs flex items-center justify-center gap-2 border border-slate-200 dark:border-zinc-700 transition-colors cursor-pointer"
              >
                <Wand2 className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" /> Régénérer de Nouvelles Variantes
              </button>
            </div>
            ...
            {generatedCopy && (
              <div className="space-y-3.5 text-xs">
                {/* Title */}
                <div className="bg-slate-50 dark:bg-[#0d0d10] border border-slate-200 dark:border-zinc-800/80 rounded-xl p-3.5 space-y-1">
                  <div className="flex items-center justify-between text-[11px] text-slate-500 dark:text-zinc-400 font-mono uppercase tracking-wider">
                    <span>Titre Recommandé</span>
                    <button
                      onClick={() => copyToClipboard(generatedCopy.title, 'title')}
                      className="text-slate-600 hover:text-slate-900 dark:text-zinc-300 dark:hover:text-white flex items-center gap-1 font-sans text-xs transition-colors cursor-pointer"
                    >
                      {copiedField === 'title' ? <CheckCheck className="w-3 h-3 text-emerald-600 dark:text-emerald-400" /> : <Copy className="w-3 h-3 text-slate-400" />}
                      {copiedField === 'title' ? 'Copié !' : 'Copier'}
                    </button>
                  </div>
                  <div className="text-slate-900 dark:text-zinc-100 font-semibold text-sm">{generatedCopy.title}</div>
                </div>

                {/* Moroccan Darija Hook */}
                <div className="bg-slate-50 dark:bg-[#0d0d10] border border-slate-200 dark:border-zinc-800/80 rounded-xl p-3.5 space-y-1">
                  <div className="flex items-center justify-between text-[11px]">
                    <span className="text-slate-700 dark:text-zinc-300 font-medium flex items-center gap-1">
                      <Flame className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" /> Accroche en Darija
                    </span>
                    <button
                      onClick={() => copyToClipboard(generatedCopy.hookDarija, 'darija')}
                      className="text-slate-600 hover:text-slate-900 dark:text-zinc-300 dark:hover:text-white flex items-center gap-1 font-sans text-xs transition-colors cursor-pointer"
                    >
                      {copiedField === 'darija' ? <CheckCheck className="w-3 h-3 text-emerald-600 dark:text-emerald-400" /> : <Copy className="w-3 h-3 text-slate-400" />}
                      {copiedField === 'darija' ? 'Copié !' : 'Copier'}
                    </button>
                  </div>
                  <div className="text-slate-900 dark:text-zinc-100 text-sm font-medium text-right leading-relaxed" dir="rtl">
                ...
                <div className="bg-slate-50 dark:bg-[#0d0d10] border border-slate-200 dark:border-zinc-800/80 rounded-xl p-3.5 space-y-1">
                  <div className="flex items-center justify-between text-[11px] text-slate-500 dark:text-zinc-400">
                    ...
                    <button
                      ...
                      className="text-slate-600 hover:text-slate-900 dark:text-zinc-300 dark:hover:text-white flex items-center gap-1 font-sans text-xs transition-colors cursor-pointer"
                    >
                      {copiedField === 'french' ? <CheckCheck className="w-3 h-3 text-emerald-600 dark:text-emerald-400" /> : <Copy className="w-3 h-3 text-slate-400" />}
                      {copiedField === 'french' ? 'Copié !' : 'Copier'}
                    </button>
                  </div>
                  <div className="text-slate-700 dark:text-zinc-300 text-xs leading-relaxed italic">
                    « {generatedCopy.hookFrench} »
                  </div>
                </div>

                {/* Guarantees & COD Reassurance */}
                <div className="bg-slate-50 dark:bg-[#0d0d10] border border-slate-200 dark:border-zinc-800/80 rounded-xl p-3.5 space-y-2">
                  <div className="text-[11px] text-slate-600 dark:text-zinc-400 font-medium flex items-center gap-1.5">
                  ...
                  <div className="space-y-1">
                    {generatedCopy.reassurancesDarija.map((r, idx) => (
                      <div key={idx} className="text-slate-800 dark:text-zinc-300 text-xs flex items-center justify-between bg-white dark:bg-zinc-900/60 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-zinc-800/60">
                ...
                {/* Bundle Offers */}
                <div className="space-y-2">
                  <div className="text-[11px] text-slate-600 dark:text-zinc-400 font-medium">Packs & Offres Dégressives Suggérés :</div>
                    ...
                    {generatedCopy.bundles.map((bundle, idx) => (
                      <div 
                        ...
                        className={`p-3 rounded-xl border ${
                          bundle.isPopular 
                            ? 'bg-white border-emerald-500/50 shadow-xs dark:bg-zinc-900 dark:border-zinc-700 dark:shadow-sm' 
                            : 'bg-slate-50 border-slate-200 dark:bg-[#0d0d10] dark:border-zinc-800/80'
                        } space-y-1 text-center`}
                      >
                        <span className={`inline-block px-2 py-0.5 rounded text-[9px] font-semibold uppercase ${
                          bundle.isPopular ? 'bg-emerald-100 text-emerald-800 dark:bg-zinc-100 dark:text-zinc-900' : 'bg-slate-200 text-slate-600 dark:bg-zinc-800 dark:text-zinc-400'
                        }`}>
                          {bundle.discountBadge}
                        </span>
                        <div className="font-medium text-slate-800 dark:text-zinc-200 text-xs">{bundle.name}</div>
                        <div className="text-slate-900 dark:text-white font-mono tabular-nums font-semibold text-sm">{bundle.priceTotal} MAD</div>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                {/* Footer Buttons */}
                <div className="flex items-center gap-2.5 pt-3.5 border-t border-slate-200 dark:border-zinc-800/80">
                  <button
                    type="button"
                    onClick={() => handleApplyAIToProduct(generatedCopy)}
                    className="flex-1 py-2 px-4 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white dark:bg-emerald-500 dark:hover:bg-emerald-400 dark:text-zinc-950 font-bold text-xs transition-colors shadow-sm flex items-center justify-center gap-2 cursor-pointer"
                  >
                  ...
                  <button
                    type="button"
                    onClick={() => setShowAICoach(false)}
                    className="py-2 px-4 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200 dark:bg-zinc-900 dark:hover:bg-zinc-800 dark:text-zinc-300 dark:border-zinc-800 font-medium text-xs transition-colors cursor-pointer"
                  >
                    Fermer
                  </button>
                </div>
            ...
            <form onSubmit={handleCreateCategory} className="space-y-3.5 text-xs">
              ...
              <div className="flex justify-end gap-2.5 pt-3 border-t border-slate-200 dark:border-zinc-800/80">
                <button
                  type="button"
                  onClick={() => setShowAddCategoryModal(false)}
                  className="px-4 py-2 rounded-lg border border-slate-200 bg-slate-100 text-slate-700 hover:bg-slate-200 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-300 dark:hover:bg-zinc-800 text-xs font-medium transition-colors cursor-pointer"
                >
                  Annuler
                </button>
                <button
                  type="submit"
                  className="px-4 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white dark:bg-emerald-500 dark:hover:bg-emerald-400 dark:text-zinc-950 font-bold text-xs transition-colors shadow-sm cursor-pointer"
                >
                  Créer la Catégorie
                </button>
              </div>
      ...
      {showReassignModal && categoryToReassign && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-[#121215] border border-slate-200 dark:border-zinc-800 rounded-xl p-6 max-w-md w-full space-y-4 shadow-2xl">
            <div className="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-zinc-800/80">
              <div className="flex items-center gap-2">
                <AlertTriangle className="w-4 h-4 text-sky-500 dark:text-sky-400" />
                <h3 className="text-base font-semibold text-slate-900 dark:text-white">Réassigner les Produits</h3>
              </div>
              <button 
                type="button"
            ...
            <form onSubmit={handleConfirmReassign} className="space-y-3.5 text-xs">
              <div className="p-3 rounded-lg bg-sky-50 dark:bg-sky-950/20 border border-sky-200 dark:border-sky-800/40 text-sky-800 dark:text-sky-300 text-xs leading-relaxed">
                La catégorie <strong className="text-slate-900 dark:text-white">« {categoryToReassign.name} »</strong> contient{' '}
                <strong className="text-slate-900 dark:text-white font-mono tabular-nums">{categoryToReassign.productCount} produit(s)</strong> rattaché(s).
                Pour éviter tout produit orphelin dans votre boutique, veuillez choisir une catégorie de destination.
              </div>

              <div>
                <label className="block text-slate-700 dark:text-zinc-300 font-medium mb-1">Catégorie de Destination :</label>
                <select
              ...
              <div className="flex justify-end gap-2.5 pt-3 border-t border-slate-200 dark:border-zinc-800/80">
                ...
                <button
                  type="submit"
                  className="px-4 py-2 rounded-lg bg-rose-600 hover:bg-rose-500 text-white font-semibold text-xs transition-colors shadow-sm cursor-pointer"
                >
