...
function FunnelContent() {
  ...
  return (
    <div className="p-6 sm:p-10 space-y-8 max-w-6xl mx-auto font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-3">
            <Filter className="w-8 h-8 text-emerald-600 dark:text-emerald-400" /> Entonnoir de Vente (Store Journey)
          </h1>
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs text-slate-700 dark:text-slate-300">
          <Clock className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400 animate-spin" />
      ...
      {currentFunnel.totalVisitors === 0 && (
        <div className="p-6 rounded-3xl bg-emerald-500/10 border border-emerald-500/20 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-bold text-slate-900 dark:text-white">Entonnoir prêt pour vos premiers visiteurs</h3>
      ...
      <div className="bg-white dark:bg-[#13171c] border border-slate-200 dark:border-slate-800/70 bento-card rounded-2xl p-6 sm:p-8 flex flex-col sm:flex-row items-center justify-between gap-6 shadow-xs">
        <div className="space-y-1 text-center sm:text-left">
          <span className="text-xs font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider">Performance Globale de la Boutique</span>
          <div className="text-3xl sm:text-4xl font-black text-slate-900 dark:text-white">
            {currentFunnel.conversionRate}% <span className="text-base font-normal text-slate-500 dark:text-slate-400">Taux de conversion global</span>
          </div>
        ...
        <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-center shrink-0">
          <div className="text-xs text-slate-500 dark:text-slate-400 font-semibold">Moyenne E-Commerce Maroc :</div>
          <div className="text-xl font-black text-emerald-600 dark:text-emerald-400 mt-0.5">8% - 12%</div>
      ...
      {/* Visual Funnel Steps */}
      <div className="bg-white dark:bg-[#13171c] border border-slate-200 dark:border-slate-800/70 bento-card rounded-2xl p-6 sm:p-8 space-y-6 shadow-xs">
        <h2 className="text-base font-extrabold text-slate-900 dark:text-white flex items-center gap-2">
          <TrendingUp className="w-5 h-5 text-sky-600 dark:text-sky-400" /> Étapes de Conversion & Déperdition de Trafic
        </h2>
              ...
              <div key={idx} className="space-y-2">
                <div className="flex items-center justify-between text-xs sm:text-sm">
                  <div className="font-bold text-slate-900 dark:text-white flex items-center gap-2">
                    <span className="w-6 h-6 rounded-full bg-slate-100 dark:bg-slate-800 text-emerald-600 dark:text-emerald-400 flex items-center justify-center text-xs font-black">
                      {idx + 1}
                    </span>
                    <span>{step.name}</span>
                  </div>
                  ...
                  <div className="flex items-center gap-4 text-xs">
                    <span className="font-mono text-slate-500 dark:text-slate-400">{step.visitors.toLocaleString()} visiteurs</span>
                    <span className="font-black text-slate-900 dark:text-white text-sm sm:text-base">{step.percentage}%</span>
                    {step.dropoff > 0 && (
                      <span className="px-2 py-0.5 rounded bg-rose-500/10 text-rose-600 dark:text-rose-400 border border-rose-500/20 text-[11px] font-bold">
                        -{step.dropoff}%
                      </span>
                    )}
                  </div>
      ...
      {currentFunnel.diagnostics.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {currentFunnel.diagnostics.map((diag, i) => (
            <div key={i} className="p-5 rounded-2xl bg-white dark:bg-[#13171c] border border-slate-200 dark:border-slate-800/70 bento-card space-y-2 shadow-xs">
              <div className="flex items-center gap-2">
                {diag.type === 'success' ? (
                  ...
                  <Lightbulb className="w-5 h-5 text-cyan-600 dark:text-cyan-400 shrink-0" />
                )}
                <h3 className="font-bold text-slate-900 dark:text-white text-xs">{diag.title}</h3>
              </div>
              <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">{diag.desc}</p>
            </div>
          ))}
        </div>
      )}
