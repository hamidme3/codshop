...
function ShippingContent() {
  ...
  useEffect(() => {
    fetch(`/api/stores/${encodeURIComponent(storeSlug)}/checkout-settings`)
      ...
      .catch((err) => console.error('[Shipping Settings] Fetch error:', err))
    ...
    <div className="p-6 sm:p-10 space-y-8 max-w-5xl mx-auto font-sans">
      ...
      <div>
        <h1 className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-3">
          <Truck className="w-8 h-8 text-emerald-600 dark:text-emerald-400" /> Paramètres Livraison & Formulaire COD
        </h1>
      ...
      <form onSubmit={handleSave} className="space-y-6">
        {/* Checkout Email Field Mode Controller */}
        <div className="bg-white dark:bg-[#13171c] border border-slate-200 dark:border-slate-800/70 rounded-2xl p-6 space-y-5 shadow-xs">
          <div className="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-slate-800">
            <div className="flex items-center gap-2.5">
              <Mail className="w-5 h-5 text-emerald-600 dark:text-emerald-400" />
              <h2 className="text-base font-extrabold text-slate-900 dark:text-white">Champ Adresse E-mail (Formulaire de Commande)</h2>
            </div>
            <span className="text-xs text-slate-500 dark:text-zinc-400">Contrôle du formulaire acheteur</span>
          </div>
              ...
              return (
                <div
                  ...
                  className={`p-4 rounded-xl border transition-all cursor-pointer flex flex-col justify-between gap-3 ${
                    isSelected
                      ? 'bg-emerald-50 dark:bg-emerald-950/20 border-emerald-500 ring-1 ring-emerald-500/50'
                      : 'bg-slate-50 dark:bg-[#0c0f12] border-slate-200 dark:border-slate-800/80 hover:border-slate-300 dark:hover:border-slate-700'
                  }`}
                >
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2">
                        <Icon className={`w-4 h-4 ${isSelected ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400 dark:text-zinc-400'}`} />
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-slate-200 dark:border-slate-800/60 text-[10px]">
                    <span className={isSelected ? 'text-emerald-600 dark:text-emerald-400 font-bold' : 'text-slate-400 dark:text-zinc-500'}>
        ...
        <div className="bg-white dark:bg-[#13171c] border border-slate-200 dark:border-slate-800/70 rounded-2xl p-6 space-y-5 shadow-xs">
          <div className="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-slate-800">
            <div className="flex items-center gap-2.5">
              ...
              <h2 className="text-base font-extrabold text-slate-900 dark:text-white">Tarifs par Zone Géographique</h2>
            </div>
            <div className="text-xs text-slate-500 dark:text-zinc-400">Calcul automatique lors de la commande</div>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
            <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-[#0c0f12] border border-slate-200 dark:border-slate-800/80 space-y-1.5">
              <label className="block text-slate-700 dark:text-zinc-300 font-semibold">Casablanca (DH) :</label>
              <input
                type="number"
                value={casaFee}
                onChange={(e) => setCasaFee(Number(e.target.value))}
                className="w-full bg-white dark:bg-[#13171c] border border-slate-200 dark:border-slate-800 rounded-lg p-2.5 text-slate-900 dark:text-white font-mono font-bold focus:border-emerald-500 focus:outline-none"
              />
              <span className="text-[10px] text-slate-400 dark:text-zinc-500">Tarif intra-muros standard</span>
            </div>

            <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-[#0c0f12] border border-slate-200 dark:border-slate-800/80 space-y-1.5">
              <label className="block text-slate-700 dark:text-zinc-300 font-semibold">Rabat & Région (DH) :</label>
              <input
                type="number"
                value={rabatFee}
                ...
                className="w-full bg-white dark:bg-[#13171c] border border-slate-200 dark:border-slate-800 rounded-lg p-2.5 text-slate-900 dark:text-white font-mono font-bold focus:border-emerald-500 focus:outline-none"
              />
              <span className="text-[10px] text-slate-400 dark:text-zinc-500">Zone Rabat-Salé-Kénitra</span>
            </div>

            <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-[#0c0f12] border border-slate-200 dark:border-slate-800/80 space-y-1.5">
              <label className="block text-slate-700 dark:text-zinc-300 font-semibold">Autres Villes (DH) :</label>
              <input
                type="number"
                ...
                className="w-full bg-white dark:bg-[#13171c] border border-slate-200 dark:border-slate-800 rounded-lg p-2.5 text-slate-900 dark:text-white font-mono font-bold focus:border-emerald-500 focus:outline-none"
              />
              <span className="text-[10px] text-slate-400 dark:text-zinc-500">Toutes les autres villes</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-[#13171c] border border-slate-200 dark:border-slate-800/70 rounded-2xl p-6 space-y-5 shadow-xs">
          <div className="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-slate-800">
            <div className="flex items-center gap-2.5">
              ...
              <h2 className="text-base font-extrabold text-slate-900 dark:text-white">Livraison Gratuite & Délais</h2>
            </div>
            <div className="text-xs text-slate-500 dark:text-zinc-400">Règles d&apos;affichage panier & checkout</div>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
            <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-[#0c0f12] border border-slate-200 dark:border-slate-800/80 space-y-1.5">
              <label className="block text-slate-700 dark:text-zinc-300 font-semibold">Seuil Livraison Offerte (DH) :</label>
              <input
                type="number"
                ...
                className="w-full bg-white dark:bg-[#13171c] border border-slate-200 dark:border-slate-800 rounded-lg p-2.5 text-slate-900 dark:text-white font-mono font-bold focus:border-emerald-500 focus:outline-none"
              />
              <span className="text-[10px] text-slate-400 dark:text-zinc-500">Toute commande &ge; à ce montant a les frais offerts (0 DH)</span>
            </div>

            <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-[#0c0f12] border border-slate-200 dark:border-slate-800/80 space-y-1.5">
              <label className="block text-slate-700 dark:text-zinc-300 font-semibold">Délai Estimé de Livraison :</label>
              <input
                type="text"
                value={deliveryTimeframe}
                onChange={(e) => setDeliveryTimeframe(e.target.value)}
                placeholder="Ex: 24h à 48h"
                className="w-full bg-white dark:bg-[#13171c] border border-slate-200 dark:border-slate-800 rounded-lg p-2.5 text-slate-900 dark:text-white font-medium focus:border-emerald-500 focus:outline-none"
              />
              <span className="text-[10px] text-slate-400 dark:text-zinc-500">Mention affichée sur le formulaire de commande</span>
            </div>
          </div>
        </div>
