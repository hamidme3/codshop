  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto pb-16">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Support & Assistance Marchand</h1>
          </div>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-sm font-medium transition shadow-xs cursor-pointer"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        {/* WhatsApp VIP */}
        <div className="bg-emerald-50/60 dark:bg-gradient-to-br dark:from-emerald-950/40 dark:via-slate-900 dark:to-slate-900 border border-emerald-200 dark:border-emerald-800/40 rounded-xl p-5 relative overflow-hidden flex flex-col justify-between shadow-xs">
          <div>
          ...
          <a
            ...
            className="flex items-center justify-center gap-2 w-full py-2 px-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-xs font-medium transition cursor-pointer"
          >
        ...
        {/* SLA Card */}
        <div className="bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 rounded-xl p-5 flex flex-col justify-between shadow-xs">
          <div>
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-blue-500/10 text-blue-600 dark:text-blue-300 border border-blue-500/20">
                Temps de Réponse
              </span>
              <Clock className="w-4 h-4 text-blue-500 dark:text-blue-400" />
            </div>
            <h3 className="text-base font-semibold text-slate-900 dark:text-white mb-1">Garantie &lt; 15 Minutes</h3>
          ...
          <div className="pt-3 border-t border-slate-200 dark:border-slate-800/80 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
            ...
            <span className="text-emerald-600 dark:text-emerald-400 font-medium">99.9% Opérationnel</span>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 rounded-xl p-5 flex flex-col justify-between shadow-xs">
          <div>
            ...
            <h3 className="text-2xl font-bold text-slate-900 dark:text-white mb-1">{openCount}</h3>
          ...
          <div className="pt-3 border-t border-slate-200 dark:border-slate-800/80 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
            ...
            <span className="text-slate-900 dark:text-white font-medium">{tickets.length} ticket(s)</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-xs">
        ...
        {loading ? (
          ...
          <div className="py-16 text-center px-4">
            <div className="w-12 h-12 rounded-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center mx-auto mb-3 text-slate-400">
              <MessageSquare className="w-6 h-6" />
            </div>
            <h3 className="text-base font-semibold text-slate-900 dark:text-white mb-1">Aucun ticket trouvé</h3>
            ...
            <button
              onClick={() => setShowModal(true)}
              className="inline-flex items-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-xs font-medium transition cursor-pointer shadow-xs"
            >
              <Plus className="w-4 h-4" />
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800">
            {filteredTickets.map((t) => {
              const dept = DEPARTMENT_LABELS[t.department] || { label: t.department, color: 'bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300' };
              const stat = STATUS_LABELS[t.status] || { label: t.status, badge: 'bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300' };
              const prio = PRIORITY_LABELS[t.priority] || { label: t.priority, color: 'text-slate-500 dark:text-slate-400' };

              return (
                <Link
                  key={t.id}
                  href={`/admin/support/tickets/${t.id}`}
                  className="p-4 hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition flex items-center justify-between gap-4 block"
                >
                  <div className="space-y-1.5 flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="font-mono text-xs text-emerald-600 dark:text-emerald-400 font-semibold">{t.ticketNumber}</span>
                    ...
                    <h4 className="text-sm font-semibold text-slate-900 dark:text-white truncate">{t.subject}</h4>
                    {t.latestMessage && (
                      <p className="text-xs text-slate-600 dark:text-slate-400 truncate max-w-2xl">
                        {t.latestMessage}
                      </p>
                    )}
                  </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-5 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-lg font-bold text-slate-900 dark:text-white">Ouvrir un Nouveau Ticket</h3>
              </div>
              <button
                onClick={() => setShowModal(false)}
                className="text-slate-400 hover:text-slate-700 dark:hover:text-white text-sm p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer"
              >
                ✕
              </button>
            </div>
            ...
            <form onSubmit={handleCreateTicket} className="space-y-4">
              ...
              <div className="flex items-center justify-end gap-3 pt-2">
                <button
                  type="button"
                  onClick={() => setShowModal(false)}
                  className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-slate-300 rounded-lg text-xs font-medium transition cursor-pointer"
                >
                  Annuler
                </button>
                <button
                  type="submit"
                  disabled={submitting}
                  className="px-5 py-2 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white rounded-lg text-xs font-semibold transition shadow-xs cursor-pointer"
                >
